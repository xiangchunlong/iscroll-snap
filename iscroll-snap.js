...
/*! iScroll-tjhf  This version by TJHF on June 20, 2016 revision */
(function () {
    ...
    iScroll = function (el, options) {
        ...
        if (that.options.useTransform)that.scroller.style[vendor + 'Transform'] = trnOpen + that.x + 'px,' + that.y + 'px' + trnClose; else that.scroller.style.cssText += ';position:absolute;top:' + that.y + 'px;left:' + that.x + 'px';
    ...
    iScroll.prototype = {
        ...
        _start: function (e) {
            ...
            if (that.options.momentum) {
                ...
                if (x != that.x || y != that.y) {
                    if (that.options.useTransition)that._unbind('webkitTransitionEnd'); else cancelFrame(that.aniTime);
                    that.steps = [];
                    that._pos(x, y);
                }
            }
        ...
        _startAni: function () {
            ...
            if (that.options.useTransition) {
                ...
                that.animating = false;
                if (step.time)that._bind('webkitTransitionEnd'); else that._resetPos(0);
                return;
            }
            animate = function () {
        ...
        stop: function () {
            if (this.options.useTransition)this._unbind('webkitTransitionEnd'); else cancelFrame(this.aniTime);
            this.steps = [];
            this.moved = false;
